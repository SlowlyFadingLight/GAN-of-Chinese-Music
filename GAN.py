...
def get_notes():
    ...
    for file in glob.glob("data_erhu/*.mid"):
...
class GAN():
    ...
    def train(self, epochs, batch_size=128, sample_interval=50):
        ...
        self.discriminator.save('dis_save_e.h5')
        self.generator.save('gen_save_e.h5')
        with open("note_e.txt", "wb") as fp: 
            pickle.dump(notes, fp)
        self.plot_loss()
        
    def generate(self,inst,name):
        if inst == 'erhu':
            self.generator = load_model('gen_save_e.h5')
            self.notes_name = 'note_e.txt'
        elif inst == 'guzhen':
            self.generator = load_model('gen_save_g.h5')
            self.notes_name = 'note_g.txt'
        
        with open(self.notes_name,'rb') as fp:
    ...
    def plot_loss(self):
        ...
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.savefig('GAN_Loss_per_Epoch_final_E.png', transparent=True)
        plt.close()
